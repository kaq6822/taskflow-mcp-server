...
`backend/app/dev/allowlist.yaml`에 명시된 argv 프리픽스만 실행 가능합니다. 미매칭 시 `policy.violation` audit + DENY.
...
- 정책 상세 구현 → `backend/app/engine/policies.py`, `backend/app/dev/allowlist.yaml`
